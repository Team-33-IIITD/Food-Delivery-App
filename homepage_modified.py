...
class viewItem(Toplevel):
    ...
    def __init__(self,restid):
        ...
        cursor=conn.cursor()
        cursor.execute('SELECT * FROM Menu WHERE restid = %s',(self.rid,))
        ind=0
    ...
    def __init__(self,rowid):
        ...
        for row in records:
            if row[0]==rowid:
                ...
                strphone="Restaurant Phone: "+str(row[2])
                stradd="Restaurant Address: "+row[3]
    ...
    def __init__(self,rowid):
        ...
        for row in records:
            if row[0]==rowid:
                ...
                stradd="Address: "+row[2]
                strmem="Membership: "+row[4]
                strcoupid="Coupon ID: "+str(row[7])
        ...
        self.label_6.place(x=100,y=200)
        Button(self, text='Place an Order',width=20,bg='brown',fg='white',command=self.database).place(x=180,y=220)
...
class restsignin(Toplevel):
  ...
  def database(self):
      ...
      conn =  mysql.connector.connect(host="remotemysql.com",database="agLYMKsBsZ",user="agLYMKsBsZ",password="LxpU9sdeQn")
      cursor=conn.cursor()
      strrest = self.restname.get()
      #cursor.execute('CREATE INDEX IF NOT EXISTS rest_email ON Restaurants(rest_name)')
      #conn.commit()
      cursor.execute('SELECT COUNT(*) FROM Restaurants WHERE rest_name = %s',(strrest,))
      records = cursor.fetchall()
      for row in records:
          cnt = row[0]
      if cnt == 0:
          self.label_1 = Label(self, text="Restaurant does not exist",width=20,font=("bold", 10))
          self.label_1.place(x=110,y=200)
      else:
          cursor.execute('SELECT * FROM Restaurants WHERE rest_name = %s',(strrest,))
          records = cursor.fetchall()
          for row in records:
  ...
  def __init__(self):
    ...
    self.label_1 = Label(self, text="Enter Restaurant name",width=20,font=("bold", 10))
...
class custsignin(Toplevel):
  ...
  def database(self):
      ...
      #cursor.execute("INSERT INTO Customers values(%s,%s,%s,-1,%s,%s)",(self.custid,self.itemid.get(),self.quant.get(),self.restid,self.size.get()))
      strcust = self.Email
...
class deliveryexecsignin(Toplevel):
  ...
  def database(self):
      ...
      cursor=conn.cursor()
      #print("INSERT INTO Cart (cust_id,dish_id,quantity,rest_id,size) values(%s,%s,%s,-1,%s,%s)",(self.custid,self.itemid.get(),self.quant.get(),self.restid,self.size.get()))
      #cursor.execute("INSERT INTO Cart values(%s,%s,%s,-1,%s,%s)",(self.custid,self.itemid.get(),self.quant.get(),self.restid,self.size.get()))
      strrest = self.delemail.get()
      
      cursor.execute('CREATE INDEX IF NOT EXISTS del_email ON DeliveryExec(delemail)')
      conn.commit()
      cursor.execute('SELECT COUNT(*) FROM DeliveryExecutive WHERE delemail = ?',(strrest,))
      records = cursor.fetchall()
      for row in records:
          cnt = row[0]
      if cnt == 0:
...
conn =  mysql.connector.connect(host="remotemysql.com",database="agLYMKsBsZ",user="agLYMKsBsZ",password="LxpU9sdeQn")
cursor=conn.cursor()
#cursor.execute('CREATE INDEX rest_e ON Restaurants(rest_name)')
#cursor.commit()
cursor.execute('ALTER TABLE Restaurants ADD INDEX (rest_name)')
